...
#     """
#     Moore–Penrose Type IV: use SymPy's built-in .pinv().
#     """
#     return A.pinv()
...
def numerical_analytical_method(input_matrix, real_matrix, imaginary_matrix, hyper_parameters):
    ...
    a = sp.Matrix([real_matrix_discretes[0].T, imaginary_matrix_discretes[0].T])
    x = a.pinv()

    X1_K = [x[0:m, 0:n].copy().T]
    X2_K = [-x[0:m, n:2 * n].copy().T]

    # Number of discretes to calculate
    # K = max(len(real_matrix_discretes), len(imaginary_matrix_discretes))

    # k is the index of current discrete being calculated
    for k in range(1, hyper_parameters["discretes_count"]):
        ...
        for i in range(1, k + 1):
            index1 = i if i < len(real_matrix_discretes) else len(real_matrix_discretes) - 1
            index2 = i if i < len(imaginary_matrix_discretes) else len(imaginary_matrix_discretes) - 1
            summa[0] += (real_matrix_discretes[index1].T * X1_K[k - i].T - imaginary_matrix_discretes[index2].T * X2_K[k - i].T)
            summa[1] += -(real_matrix_discretes[index1].T * X2_K[k - i].T + imaginary_matrix_discretes[index2].T * X1_K[k - i].T)
        x1 = -X1_K[0].T * summa[0] + X2_K[0].T * summa[0]
        X1_K.append(x1.T)
        x2 = -X1_K[0].T * summa[1] + X2_K[0].T * summa[1]
        X2_K.append(-x2.T)
